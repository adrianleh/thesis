...
, as described in \cref{sec:impl:fixup}, and unroll the loop a set number of times.
The unrolling process is covered in \cref{sec:impl:unroll}.
...
We select this form, as most loops follow the form of using a counter or iterating over a given container, which basically condenses down to this form.
...
These assumptions need to be checked before the optimization is applied, to ensure that the optimization is only applied, iff all conditions are met, such that its transformed product will be semantically equivalent.

In the case of the optimization discussed, in~\cref{sec:impl} we laid out the structured of the targeted loops.
This section formalizes these requirements and extends them, such that the further unrolling process conserves semantics.

Firstly, as we use the existing loop unrolling functionality as a sub-step (see~\cref{sec:impl:unroll}), it needs to be ensured, that the respective~\libFIRM-graph is in LCSSA form.
...
On the contrary, $N$, the bound, has to be loop-invariant, which means that it does not change through the entire evaluation of the loop, as it is checked against every iteration.
...
Further, any calls, must either have functions known at compile time and neither have aliasing stores or aliasing parameters, or mean the loop cannot be unrolled with a loaded bound, as these called functions might potentially have aliasing stores in them.

Lastly the unroll-factor -- meaning how often the loop body is copied inside the unrolled loop -- $f$, and increment $c$, are restricted, such that $t_{min} \leq c \cdot (f+1) \leq t_{max}$, where $t_{min}$ is the minimum value of the integer type of $c$ and $t_{max}$ the respective maximum.
Hence, we prevent $c \cdot (f + 1)$ from overflowing, which will turn out to be important in \cref{sec:impl:fixup:header-cond} and \cref{sec:impl:fixup:duff} and further discussed.
Further, by hence forcing $c$ to be a compile-time constant, $c$ is certainly loop invariant.
...
It is worth mentioning, that the unrollability is only checked, if the current loop unrolling mechanism determines that it cannot be applied.
We chose this design, as statically unrolling without any further fixup code, inherently simplifies the control flow and hence should yield better, or at least equal, performance.
...
Especially to be noted is that in \cref{fig:impl:unroll:unroll-factor-2-after} we duplicate the loop header, and that hence the number of conditions did not decrease through the loop unroll.
...
Unfortunately though in the use cases of an unknown bound the constant bit analysis, of course, does not work, as~\libFIRM~cannot recognize the additional semantics that are implicitly affixed to the transformed loop.
...
First, we rewire all phis in the header, such that all in-loop nodes descending from any given phi node each get the in-loop predecessors of the phi node as predecessors themselves, whilst the phi node is no longer a predecessor of any of the descendants.
...
In this section we did not consider the fixup code needed, but rather plainly focussed on unrolling the loop.
\Cref{sec:impl:fixup:duff} uses a generalized version of duff's device to create the needed fixup, whereas in \cref{sec:impl:fixup:loop} a copy of the original loop.
...
We then induct this pattern for $\Mloop = M - nf - b, n \in \mathbb{N}_{+}, b \in \lbrack 0, f \lbrack \andinN$.
...
These equations are especially significant in~\cref{sec:impl:fixup:duff}.
...
To combat this, we set the bound of the unrolled loop to $\hat{N} = N - c \cdot (f - 1)$.~\footnote{N.B.: $c \cdot f$ does not overflow, as per preconditions}
...
Using this restriction it can be asserted that $c \times (f + 1) \in \lbrack t_{min}, t_{max} \rbrack$ and therefore does not overflow.
...
After this, we will wire all duplicated blocks such that, they be reachable by conditions, like those previously described.
...
\Cref{alg:impl:fixup:duff:create-fixup} shows how we create this structure in~\libFIRM.
...
The, admittedly straightforward, algorithm tries to find a factor $f = 2^n, n \in \mathbb{N}_{>0}$, that minimizes the euclidean distance between the unrolled size ($= f \cdot \text{original size}$), and a pre-determined maximum size.
