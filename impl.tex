...
These assumptions will be assured, by checking corresponding preconditions before the optimization is applied, to ensure that the optimization is only applied, if all conditions are met, such that its transformed product will be semantically equivalent.
...
Therefore, while it is a preliminary step, assuring LCSSA form can never be a hindrance to unrolling.
...
Seeing that there is an explicit entry point for the loop, the header, there are no required preconditions for $I$, considering it, therefore, is only once evaluated before the loop, but inherently not determining of the control flow after the initial evaluation.
On the contrary,  $N$, the bound, has to be loop-invariant, which means that it may not change through the entire evaluation of the loop, because it is checked against every iteration.
...
Concluding, iff $N$ is loop-invariant, the bound checks can be performed less often, while keeping the original semantics intact.
...
With the previous usage, this was not an issue though, due to using its constant bit analysis,~\libFIRM~would automatically remove these excess headers~\cite{aebi18bachelorarbeit}.
...
First, we rewire all $\phi$-nodes in the header, such that all in-loop nodes descending from any given $\phi$-node each get the in-loop predecessors of the $\phi$-node as predecessors themselves, while the $\phi$-node is no longer a predecessor of any of the descendants.
...
In this section, we did not consider the fixup code needed, but instead plainly focused on unrolling the loop.
\Cref{sec:impl:fixup:duff} uses a generalized version of Duff's device to create the required fixup, whereas in \cref{sec:impl:fixup:loop} a copy of the original loop will be used.
...
The primary identity that is to be conserved, to retain the original semantics, is shown above in \cref{eqn:impl:fixup:duff:conserve-semantics-identity}.
...
Conjecture: $\Mfixup \in \left \rbrack 0,  f \right \rbrack$.\\
...
For the following mathematical consideration, we need to round up in integer division.
...
We know that $c \cdot (f + 1)$ cannot over- or underflow, as per the preconditions layed out in \cref{sec:impl:unrollability}.
Since $f > 0$, $c \cdot (f - 1)$ will therfore also not overflow.
Though, $N - c \cdot (f - 1)$ can still over- or underflow, due to the subtraction of $c \cdot (f - 1)$ from $N$, and there hence is nevertheless a possibility to construct an example where this change does not conserve semantics.
...
For if an under- or overflow is detected, the control flow will jump directly the fixup code.
